...
const BlacklistValidatorAbstraction = artifacts.require('BlacklistValidator');

contract('BlacklistValidator', function(accounts) {
  const from = accounts[0];
  const investor1 = accounts[1];
  let instance;

  beforeEach( async () => {
    instance = await BlacklistValidatorAbstraction.new({from});
  });
  ...
  describe('.getName()', () => {
    const TRANSFER_VALIDATOR_NAME = 'BlacklistValidator';
  ...
  describe('.addAddress()', async () => {
    ...
    it('does not allow non-owner to add to blacklist', async () => {
  ...
  describe('.canSend()', async () => {
    it('it returns true when the _to address is blacklisted', async () => {
    ...
    it('it returns false even when the _from address is not blacklisted', async () => {
      const errMsg = 'It should report false for blacklisted _to address';
      await instance.addAddress(investor1);
    ...
    it('it returns true when the _to address is not blacklisted', async () => {
      const errMsg = 'It should report true for invalid address';
      const isBlacklisted = await instance.isBlacklisted.call(investor1);
      assert.isFalse(isBlacklisted, 'Should not be on the blacklist');
    ...
    it('it returns true for non-blacklisted _to even when the _from address is blacklisted', async () => {
      const errMsg = 'It should report true for not blacklisted _to address';
      await instance.addAddress(from);
      const isBlacklisted = await instance.isBlacklisted.call(from);
      assert.isTrue(isBlacklisted, 'From should be on the blacklist');
      const isBlacklistedInvestor = await instance.isBlacklisted.call(investor1);
      assert.isFalse(isBlacklistedInvestor, 'Should not be on the blacklist');
      const result = await instance.canSend.call(
        0x00,
        from,
        investor1,
