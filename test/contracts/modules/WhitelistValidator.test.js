...
const WhitelistValidatorAbstraction = artifacts.require('WhitelistValidator');

contract('WhitelistValidator', function(accounts) {
  const from = accounts[0];
  const investor1 = accounts[1];
  let instance;

  beforeEach( async () => {
    instance = await WhitelistValidatorAbstraction.new({from});
  });
  ...
  describe('.getName()', () => {
    const TRANSFER_VALIDATOR_NAME = 'WhitelistValidator';
  ...
  describe('.canSend()', async () => {
    it('it returns true when the _to address is whitelisted', async () => {
    ...
    it('it returns true even when the _from address is not whitelisted', async () => {
    ...
    it('it returns false when the _to address is not whitelisted', async () => {
